...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 29 12:31:57 2025
...
# --- 2. Defining the NEW Interpolation Model (9 Parameters - Added Harmonic) ---

def two_scale_sine_model(t, C0, C_annual, phi_annual, A_base, A_seasonal, phi_amp, phi_diurnal, A_harm, phi_harm):
    """
    9-parameter model: Combines annual trend, a seasonally-modulated diurnal 
    (24h) cycle, AND a 12-hour harmonic component for better fit fidelity.
    """
    ...
    # 3. Second Diurnal Harmonic (12-hour cycle) - NEW TERM
    harmonic_component = A_harm * np.sin(2 * np.pi * t / 12 + phi_harm)
    
    return mu_t + diurnal_component + harmonic_component


# --- 3. Curve Fitting Execution ---

# Updated Initial Guesses (9 parameters now)
# [C0, C_annual, phi_annual, A_base, A_seasonal, phi_amp, phi_diurnal, A_harm (NEW), phi_harm (NEW)]
initial_guess = [10.0, 6.0, 0.0, 2.0, 1.0, 0.0, -1.0, 0.5, 0.0] 

try:
    print("\nBeginning curve fitting process with 9-parameter model (including 12h harmonic)...")
    popt, pcov = curve_fit(
        two_scale_sine_model, 
        time_hours, 
        temp_data, 
        p0=initial_guess,
        maxfev=15000 # Increased iterations again for the more complex model
    )
    
    # Extract fitted parameters (9 variables now)
    C0_fit, C_annual_fit, phi_annual_fit, A_base_fit, A_seasonal_fit, phi_amp_fit, phi_diurnal_fit, A_harm_fit, phi_harm_fit = popt

    print("\n--- Fitted Parameters (Advanced Mathematical Model with Harmonic) ---")
    ...
    equation_string = (
        ...
        f"T(t) = Annual_Trend(t) + Diurnal_Fundamental(t) + Diurnal_Harmonic(t)\n"
        f"T(t) = {term_A}\n"
...
plt.title('2023 Edinburgh Temperature: Full Year Interpolation (9-Parameter Harmonic Model)', fontsize=16)
